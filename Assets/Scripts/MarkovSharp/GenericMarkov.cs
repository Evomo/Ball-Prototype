...
namespace MarkovSharp
{
    ...
    public abstract class GenericMarkov<TPhrase, TUnigram> : IMarkovStrategy<TPhrase, TUnigram>
    {
        ...
        {
            Debug.Log($"Learning phrase: '{phrase}'");
            ...
            {
                Debug.Log($"Phrase {phrase} too short - skipped");
                return;
            }
            ...
            {
                 Debug.Log($"Adding phrase {phrase} to source lines");
            ...
            for (var j = Level; j > 0; j--)
            {
                ...
                try
                {
                    previous = tokens[tokens.Length - j];
                     Debug.Log($"Adding TGram ({typeof(TUnigram)}) {previous} to lastCol");
                    lastCol.Add(previous);
                }
                catch (IndexOutOfRangeException e)
                {
                    Debug.LogWarning($"Caught an exception: {e}");
                    previous = GetPrepadUnigram();
                    lastCol.Add(previous);
                }
            }

             Debug.Log($"Reached final key for phrase {phrase}");
        ...
        public void Retrain(int newLevel)
        {
            ...
            Debug.Log($"Retraining model as level {newLevel}");
            Level = newLevel;
        ...
        public IEnumerable<TPhrase> Walk(int lines = 1, TPhrase seed = default(TPhrase))
        {
            ...
            Debug.Log($"Walking to return {lines} phrases from {Chain.Count} states");
            if (lines < 1)
            {
            ...
            while (created < lines)
            {
                ...
                {
                    Debug.Log($"Breaking out of walk early - {genCount} generations did not produce {lines} distinct lines ({sentences.Count} were created)");
                    break;
                }
